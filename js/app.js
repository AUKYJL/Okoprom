 ...
 */
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".menu-btn")) {
                document.addEventListener("click", (function(e) {
                    const iconMenu = e.target.closest(".menu-btn");
                    if (bodyLockStatus && iconMenu) {
                        bodyLockToggle();
                        if (!document.documentElement.classList.contains("menu-open")) {
                            const menu = document.querySelector(".menu");
                            setTimeout((() => {
                                const iconMenuToTopPx = iconMenu.getBoundingClientRect().top;
                                iconMenu.style.top = `${iconMenuToTopPx}px`;
                                console.log(iconMenuToTopPx);
                                menu.append(iconMenu);
                            }), 400);
                        } else {
                            const bottomContent = document.querySelector(".bottom-header__content");
                            iconMenu.style.top = `0px`;
                            bottomContent.append(iconMenu);
                        }
        ...
        if (inputMasks.length) modules_flsModules.inputmask = Inputmask("+ 9 (999) 999 99").mask(inputMasks);
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                new core(".slider-body-main-home", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                    ...
                    speed: 800,
                    navigation: {
                        prevEl: ".bottom-slider-body-main-home__arrow_left",
                        nextEl: ".bottom-slider-body-main-home__arrow_right"
                    },
                    on: {}
                });
                new core(".slider-tabs-info-product", {
                    observer: true,
                    observeParents: true,
                    slidesPerView: "auto",
                    spaceBetween: 15,
                ...
                new core(".slider-see-also-product", {
                    observer: true,
                    observeParents: true,
                    slidesPerView: "auto",
                    spaceBetween: 15,
        ...
        function addListenersToBtns() {
            ...
            if (filterBtns) document.addEventListener("click", (e => {
                if (e.target.closest("._filter-btn")) {
                    ...
                    if (filterBtn.closest(".left-production-catalog__list")) {
                        const filterBtnArr = filterBtn.closest(".left-production-catalog__list").querySelectorAll("li");
                        const btnsFilters = [ ...filterBtnArr ].map((el => el.getAttribute("data-filter")));
                        btnsFilters.forEach((fltr => {
                            currentFilters = currentFilters.filter((el => el !== fltr));
                        }));
                    }
                    if (!currentFilters.includes(filter)) currentFilters.push(filter); else currentFilters = currentFilters.filter((el => el !== filter));
        ...
        function setCardsVisible(currentFilters) {
            ...
            cards.forEach((card => {
                ...
                if (currentFilters.every((filter => cardFilters.includes(filter)))) card.removeAttribute("hidden"); else card.setAttribute("hidden", "");
            }));
        }
        function draggableSlideDownMenu() {
            const draggableEl = document.querySelector(".list-left-production-catalog__slide-down");
            ...
            function moveTouch(event) {
                ...
                if (newHeight / startHeight < .7) canClose = true; else canClose = false;
                if (newHeight <= startHeight + .2 * startHeight) listBody.style.height = newHeight + "px";
            }
            function endTouch() {
                if (canClose) closeFilters(listBody, darkBG, productionCatalog, startHeight);
        ...
        function closeFilters(listBody, darkBG, productionCatalog, startHeight) {
...
*/
        ...
        const tlLinks = gsapWithCSS.timeline({
            paused: true
        });
        let tlSearch;
        function createTlSearch() {
            const tlSearch = gsapWithCSS.timeline({
                paused: true
            });
            tlSearch.to(".right-body-header__btn, .right-body-header__link._icon-cart", {
                xPercent: 100,
                autoAlpha: 0,
                duration: .05
            ...
            }).fromTo(".form-right-body-header__close", {
                xPercent: 100,
                autoAlpha: 0,
                display: "none"
            }, {
                ...
                display: "flex",
                duration: .1
            }, "+=0.1").to(".form-right-body-header__input", {
                x: 10,
                width: GetSearchFieldWidth,
                padding: "12px 20px",
                duration: .2,
                onUpdate: function() {
                    if (!tlSearch.reversed()) this.targets()[0].style.width = GetSearchFieldWidth();
                }
            });
            return tlSearch;
        }
        ...
        document.querySelector(".right-body-header__btn");
        document.querySelector(".form-right-body-header__link._icon-cart");
        ...
        function GetSearchFieldWidth() {
            const bottomHeaderContLeft = document.querySelector(".bottom-header__content").getBoundingClientRect().left;
            const searchBtnLeft = document.querySelector("#header-search").getBoundingClientRect().left;
            return `${searchBtnLeft - bottomHeaderContLeft}px`;
        }
        function MoveFormToRight(el) {
            const contRight = document.querySelector(".bottom-header__content").getBoundingClientRect().right;
            const elRight = el.getBoundingClientRect().right;
            return contRight - elRight;
        }
        document.addEventListener("click", (e => {
            if (e.target.closest("#header-search")) if (!headerSearch.classList.contains("active")) {
        ...
        window.addEventListener("resize", (e => {
            if (headerSearch.classList.contains("active")) document.querySelector(".form-right-body-header__input").style.width = GetSearchFieldWidth();
        }));
        document.querySelector(".advantages-catalog");
        const advantagesCards = document.querySelectorAll(".advantages__cards");
        const productsCount = document.querySelector(".top-production-catalog__product-count");
        if (productsCount) countProducts();
        if (advantagesCards) advantagesCards.forEach((e => {
            const activeCardsCount = e.getAttribute("data-active-cards-count");
            const cards = e.children;
            for (let i = 0; i < activeCardsCount; i++) cards[i].classList.add("_active");
        }));
        function countProducts() {
            const products = document.querySelectorAll(".card-right-production-catalog");
            productsCount.querySelector("span").innerHTML = [ ...products ].reduce(((acc, el) => {
        ...
        function NewBtnText(filterBtn, ...els) {
            ...
            let elsText;
            if (tagName === "input") {
                if (els.some((el => el.value === ""))) return originalText;
            ...
            } else elsText = els.map((el => {
                const checkbox = el.closest(".radio-form-body-production-filters").querySelector("input");
                return checkbox.checked ? el.innerHTML : null;
            })).filter((el => el !== null));
            console.log(elsText);
            return `${originalText}: ${elsText[0]}` + (elsText[1] ? ` - ${elsText[1]}` : "");
        }
        ...
        document.addEventListener("click", (e => {
            if (e.target.closest(".filters-right-production-catalog__item")) e.target.classList.toggle("_active");
            if (e.target.closest(".filters-right-production-catalog-choose")) {
                const itemsChoose = document.querySelectorAll(".filters-right-production-catalog-choose");
                let isActive = false;
                if (e.target.closest(".filters-right-production-catalog-choose__title")) {
                    if (e.target.closest(".filters-right-production-catalog-choose__title").classList.contains("_active")) isActive = true;
                    for (const item of itemsChoose) {
                        item.querySelector(".filters-right-production-catalog-choose__title").classList.remove("_active");
                        item.querySelector(".body-production-filters").classList.remove("_active");
                    }
                }
                if (!e.target.closest(".body-production-filters")) if (!isActive) {
                    const itemChoose = e.target.closest(".filters-right-production-catalog-choose");
                    itemChoose.querySelector(".filters-right-production-catalog-choose__title").classList.toggle("_active");
                    itemChoose.querySelector(".body-production-filters").classList.toggle("_active");
                }
                countProducts();
            }
            if (productsCount && !e.target.closest(".filters-right-production-catalog-choose")) {
                const itemsChoose = document.querySelectorAll(".filters-right-production-catalog-choose");
                for (const item of itemsChoose) {
                    item.querySelector(".filters-right-production-catalog-choose__title").classList.remove("_active");
                    item.querySelector(".body-production-filters").classList.remove("_active");
            ...
            if (e.target.closest(".list-left-production-catalog__item")) {
                ...
                const filterBtn = document.querySelector(".body-production-catalog__filter-btn");
                filterBtn.innerHTML = curEl.innerHTML;
                if (prodCatalog.classList.contains("_active")) execCloseFilters();
                for (const el of itemsList.children) el.classList.remove("_active");
                if (curEl.getAttribute("data-filter") !== "all") document.querySelector(".filters-right-production-catalog").classList.add("_active"); else {
                    document.querySelector(".filters-right-production-catalog").classList.remove("_active");
            ...
            if (e.target.closest(".body-production-filters__btn")) {
                ...
                const filterBtn = item.querySelector(".filters-right-production-catalog-choose__title");
            ...
            if (e.target.closest(".body-production-catalog__filter-btn")) {
                ...
                document.querySelector(".list-left-production-catalog").classList.add("_active");
                document.querySelector(".left-production-catalog__dark-bg").classList.add("_active");
                bodyLock();
            }
            if (e.target.closest(".left-production-catalog__dark-bg")) execCloseFilters();
        }));
        window["FLS"] = true;
        isWebp();
        menuInit();
